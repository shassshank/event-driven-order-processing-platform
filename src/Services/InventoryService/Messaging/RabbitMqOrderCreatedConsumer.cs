...
public sealed class RabbitMqOrderCreatedConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _channel.QueueDeclare(
            RabbitMqTopology.Queues.InventoryOrderCreated,
            durable: true,
            exclusive: false,
            autoDelete: false,
            arguments: arguments);
        ...
        _channel.BasicConsume(
            queue: RabbitMqTopology.Queues.InventoryOrderCreated,
            autoAck: false,
            consumer: consumer);
    ...
    private async Task HandleMessageAsync(object sender, BasicDeliverEventArgs args)
    {
        ...
        try
        {
            ...
            var envelope = JsonSerializer.Deserialize<EventEnvelope<OrderCreated>>(json, JsonOptions)
                ?? throw new JsonException("OrderCreated envelope could not be deserialized.");

            if (!string.Equals(envelope.EventType, nameof(OrderCreated), StringComparison.Ordinal))
            {
                throw new JsonException($"Unexpected event type '{envelope.EventType}'.");
            }

            using var scope = _scopeFactory.CreateScope();
            var handler = scope.ServiceProvider.GetRequiredService<InventoryReservationService>();
            await handler.HandleOrderCreatedAsync(envelope, CancellationToken.None);
        ...
        catch (JsonException ex)
        {
            RabbitMqMessageFailureHandler.DeadLetter(
                ...
                "inventory.failed",
                _logger,
                "Poison OrderCreated message could not be deserialized.");
        }
        catch (Exception ex)
        {
