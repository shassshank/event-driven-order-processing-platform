...
public sealed class RabbitMqOrderCancelledConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _channel.QueueDeclare(
            RabbitMqTopology.Queues.InventoryOrderCancelled,
            durable: true,
            exclusive: false,
            autoDelete: false,
            arguments: arguments);
        ...
        _channel.QueueDeclare(
            RabbitMqTopology.DeadLetterQueues.InventoryFailed,
            durable: true,
            exclusive: false,
            autoDelete: false);
        _channel.QueueBind(
        ...
        _channel.BasicConsume(
            queue: RabbitMqTopology.Queues.InventoryOrderCancelled,
            autoAck: false,
            consumer: consumer);
    ...
    private async Task HandleMessageAsync(object sender, BasicDeliverEventArgs args)
    {
        ...
        catch (JsonException ex)
        {
            RabbitMqMessageFailureHandler.DeadLetter(
                ...
                "inventory.failed",
                _logger,
                "Poison OrderCancelled message could not be deserialized.");
        }
        catch (Exception ex)
        {
