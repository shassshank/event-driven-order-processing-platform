...
public sealed class OutboxPublisher
{
        ...
        foreach (var message in messages)
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Published payment outbox message {OutboxMessageId} with message id {MessageId} after {PublishAttempts} failed attempts.",
                    message.Id,
                    message.MessageId,
            ...
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                ...
                _logger.LogError(
                    ex,
                    "Failed to publish payment outbox message {OutboxMessageId} with message id {MessageId}. Attempt {PublishAttempts}/{MaxPublishAttempts}.",
                    message.Id,
                    message.MessageId,
                    message.PublishAttempts,
    ...
    private async Task<IReadOnlyList<long>> ClaimNextBatchAsync(CancellationToken cancellationToken)
    {
        ...
        return await _dbContext.Database.SqlQuery<long>($"""
            update payment_outbox_messages
            set "Status" = 'Processing',
                "ProcessedOnUtc" = {now},
                "Error" = null
            where "Id" in (
                select "Id"
                from payment_outbox_messages
                where
                    "Status" = 'Pending'
